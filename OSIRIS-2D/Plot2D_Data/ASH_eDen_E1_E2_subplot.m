...
%     [xIndMax_qe,yIndMax_qe]=ind2sub(size(dset_qe),...
%         find(dset_qe==max(max(dset_qe,[],1),[],2),1));
%     Ngyby2qe=yIndMax_qe;
%     LOqe=dset_qe(:,Ngyby2qe);
%     
...
%     LOe1=dset_e1(:,Ngyby2e1);
    ...
    maxabse=max(abs([maxe1;mine1;maxe2;mine2])); 
...
%%%%%% this section plots E1X1 and P1X1 %%%%%%%%%%%%%%%%%%%%%%%    
    ...
    H1=plot(xg,LOe1); hold on;
    H2=plot(xg,LOe2);   hold off;
    
    set(gca,'xlim',[xlim1 xlim2]);
    set(gca,'ylim',[ylim1cha ylim2cha]);
%     set(gca,'ytick',etick);
%     set(gca,'ylim',[10 80]);
%     set(gca,'ytick',linspace(10,80,8));
...
%         cbfreeze(colorbar('north'));
    str_tmp1=strcat('Frame:',num2str(frno),'     : ',...
        ...
        strcat('Dist=',num2str(time*t_nor*vel_c*1e4),'(',...
        '\mu m',')'));
    title(str_tmp1);
