...
%     
    ...
    x1minene=x1lt_ene1(1); x1maxene=x1lt_ene1(2);
    x2minene=x2lt_ene1(1); x2maxene1=x2lt_ene1(2);
    [Ngx_LOene,Ngy_LOene]=size(dset_ene1);
    Ngyby2ene=round(Ngy_LOene/2);
    LOene_dat=dset_ene1(:,Ngyby2ene);
...
%     subplot(121)
    E=zeros(Ngx_LOene,1);
     E1=zeros(Ngx_LOene,1);
   E(:)=squeeze(dset_ene(45,:));
...
%      plot((kx),abs(F3)/(Ngx_LOb3*dx_b3),'k','LineWidth',2);hold on;
   %----1sided------%
    kx=(0:nfft/2-1)/(dx_ene*nfft);	 	 
 loglog((kx),smooth(abs( F3(nfft/2+1:nfft))/(Ngx_LOene*dx_ene)),'k','LineWidth',1);hold on;
...
%  print('autoExample', '-depsc2', '-r300');
 ...
 title(strcat('FFT Spectra (KE), t= ',num2str(time_ene1)));
 xlim([0.01 25])
