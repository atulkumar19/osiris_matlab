...
aa=zeros(length(frm),1);
aa2=zeros(length(frm),1);
% Get the width and height of the figure
%============AVI-FILE-NAME==========================
 aviobj = avifile ( 'mean-eden-b-cur', 'fps',5); 
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
for ii=1:length(frm)   
    ...
    mean1_J1=mean(dset_J,1);
    mean1_qe=mean(dset_qe,1);
    
    aa(ii)=max(mean1_qe);
    aa2(ii)=min(mean1_qe);
    subplot(2,2,1)
    plot(yg_J1,mean1_J1,'b','LineWidth',2);
    ...
    ylabel('J_x(n_0ec)');
    subplot(2,2,2)
    plot(time_b3,(dset_b3)*(dset_b3)','b','LineWidth',2);
     set(gca,'linewidth',2.0,'fontsize',20,'fontweight','b');
