...
pth0='/home/atul/OSIRIS/MS25-0.5';
...
%============AVI-FILE-NAME==========================
writerObj = VideoWriter ('phase_space_p1p2.avi');
...
%      nBins_x = 5025;
%  nBins_y = 500;
%   [counts, bin_centers] = hist3([p1_raw(:) p2_raw(:)], [nBins_x nBins_y]);
%   x_bin_centers = bin_centers{1};
%   y_bin_centers = bin_centers{2};
%   pcolor( y_bin_centers,x_bin_centers,  log(counts));shading('interp');colorbar;hold on;
   plot(p1_raw(1:10:length(p1_raw)),p2_raw(1:10:length(p2_raw)),'.b');
   hold on
   th = 0:pi/50:2*pi;
    p0=0.2;
    px1 = p0 * cos(th) ;
...
hold off
   ...
   xlim([-3 3]);
    ...
    title(strcat('Phase-Space Plot(Ions), t=',num2str(time_qe)));
