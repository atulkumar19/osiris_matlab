...
%============AVI-FILE-NAME==========================
 aviobj = avifile ( 'mean-b-field', 'fps',5); 
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
for ii=1:length(frm)   
...
%     dset_J=dset_J1+dset_J2+dset_J3;
    ...
    mean1_b1=mean(dset_b1,1);
    mean1_b2=mean(dset_b2,1);
    mean1_b3=mean(dset_b3,1);
    
    subplot(2,2,1)
    plot(yg_b1,mean1_b1,'b','LineWidth',2);
    set(gca,'linewidth',2.0,'fontsize',20,'fontweight','b');
     title(strcat('mean-B_x-field, t= ',num2str(time_qe)));
      xlabel('x2(c/\omega_{pe})');
    
    ylabel('B_x(mc\omega_{pe}/e)');
    subplot(2,2,2)
    plot(yg_b2,mean1_b2,'b','LineWidth',2);
     set(gca,'linewidth',2.0,'fontsize',20,'fontweight','b');
    title(strcat('mean-B_y-field, t= ',num2str(time_qe)));
      xlabel('x2(c/\omega_{pe})');
    
    ylabel('B_y(mc\omega_{pe}/e)');
    subplot(2,2,[3 4])
    plot(yg_b3,mean1_b3,'b','LineWidth',2);
     set(gca,'linewidth',2.0,'fontsize',20,'fontweight','b');
      title(strcat('mean-B_z-field, t= ',num2str(time_qe)));
      xlabel('x2(c/\omega_{pe})');
