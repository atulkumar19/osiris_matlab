...
for n=0:npts/2-1
  ...
  t = 2*n*res+oddwin;
  R(1) = z(t)*conj(z(t)) + 1i*z(t+res)*conj(z(t+res));
  v1 = z(t+idx).*conj(z(t-idx));
  v2 = z(t+res+idx).*conj(z(t+res-idx));
  R(idx+1) = v1+1i*v2;
  R(win-idx+1) = conj(v1)+1i*conj(v2);
...
function hil = MyHilbert(s)
    s = s(:);
    fft_s = fft(s);
    n = numel(s);
    nyq = floor(n/2+1);
    if 2*fix(n/2) == n
        fft_s = [0;-1i*fft_s(2:nyq-1);0;1i*fft_s(nyq+1:end)];
    else
        fft_s = [0;-1i*fft_s(2:nyq);1i*fft_s(nyq+1:end)];
    end
